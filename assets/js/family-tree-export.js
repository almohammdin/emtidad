...
function buildFrame(){
  ...
  const width=Math.max(1000,Math.min(2800,stage.scrollWidth+90));
  ...
  const clone=stage.cloneNode(true);clone.style.width=`${Math.max(900,stage.scrollWidth)}px`;clone.style.height='auto';clone.querySelector('.ft-canvas')?.style.setProperty('min-width','0');clone.querySelector('.ft-canvas')?.style.setProperty('width',`${Math.max(860,document.getElementById('canvas')?.scrollWidth||860)}px`);clone.querySelector('.ft-empty')?.remove();frame.querySelector('.tree-export-body').appendChild(clone);document.body.appendChild(frame);return frame;
}
async function renderCanvas(){
  ...
  try{const w=frame.scrollWidth,scale=w>2200?1.55:w>1600?1.9:2.35;return await html2canvas(frame,{backgroundColor:'#ffffff',scale,useCORS:true,logging:false,width:frame.scrollWidth,height:frame.scrollHeight,windowWidth:frame.scrollWidth,windowHeight:frame.scrollHeight,scrollX:0,scrollY:0});}finally{frame.remove();}
}
