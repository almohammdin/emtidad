(()=>{
'use strict';
const $=id=>document.getElementById(id),EXAMPLE_KEY='emtidad_tree_example_open';
function updateClearLabel(){const btn=$('clearTreeBtn');if(btn)btn.textContent=sessionStorage.getItem(EXAMPLE_KEY)?'حذف المثال والبدء من جديد':'البدء من جديد';}
function clearTree(){
  ...
  sessionStorage.removeItem(EXAMPLE_KEY);
  location.reload();
}
async function saveImage(){
  ...
  const btn=$('imageBtn'),old=btn.textContent;btn.disabled=true;btn.textContent='جار تجهيز الصورة';
  try{
...
$('sampleBtn')?.addEventListener('click',()=>setTimeout(()=>{const names=[...document.querySelectorAll('.ft-name b')].map(x=>x.textContent.trim());if(names.includes('عبدالله')&&names.includes('نورة')){sessionStorage.setItem(EXAMPLE_KEY,'1');updateClearLabel();}},0));
