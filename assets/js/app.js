...
const statsData=[
 ...
 {value:'25%',title:'حققت معدل نمو مضاعف',meaning:'يربط التقرير النمو بوضوح الغرض والمرونة والاستثمار المدروس.',region:'62 دولة ومنطقة',sample:'1,325 شركة عائلية',year:'2025',source:'PwC',kind:'مسح مهني',url:'https://www.pwc.com/gx/en/issues/business-model-reinvention/family-business-survey.html'},
...
const toolsData=[
 ...
 {title:'سياسة توظيف أفراد العائلة',icon:'family',kind:'مسودة سياسة',desc:'بنود أساسية للمؤهلات والخبرة والتقييم.',intro:'هيكل تمهيدي لصياغة سياسة عادلة ومعلنة للعمل العائلي داخل الشركة.',sections:[
  ...
  {title:'أثناء العمل',items:['تحديد المدير المباشر بما يقلل تضارب المصالح','ربط الأجر والمزايا بسلم معتمد','تقييم الأداء وفق مؤشرات موثقة','تحديد مسار الترقية والتطوير المهني']},
 ...
 {title:'قائمة التعاقب القيادي',icon:'succession',kind:'قائمة تحقق',desc:'تقييم جاهزية القيادة والبدائل والتأهيل.',intro:'قائمة لتحديد الوظائف الحرجة وتجهيز انتقال القيادة وتنظيمه.',sections:[
  ...
  {title:'الجاهزية',items:['متطلبات الدور ومعايير النجاح مكتوبة','يوجد بديل طارئ عند الغياب المفاجئ','المرشحون المحتملون محددون بموضوعية','فجوات كل مرشح موثقة','خطة التطوير والتجربة داخل العمل معتمدة']},
...
const questions={
 purpose:[
  ...
  {id:'values',q:'هل توجد قيم عائلية محددة تتحول إلى معايير قرار وسلوك؟',d:'تظهر في التوظيف والاستثمار والخلافات والتمثيل.',w:2,doc:'مدونة القيم والسلوك العائلي',action:'تحويل القيم إلى مبادئ واضحة قابلة للمراجعة.'},
...
const answerOptions=[
 {label:'غير موجود',value:0},{label:'موجود دون توثيق',value:1},{label:'قيد الإعداد',value:2},{label:'موثق ومعتمد',value:3},{label:'مطبق وتتم متابعته',value:4},{label:'لا ينطبق',value:null}
];
const RESULT_LEVELS=[
 ...
 {min:45,max:64,label:'قواعد تحتاج إلى استكمال',desc:'جزء مهم من المنظومة واضح، وتحتاج بقية القواعد إلى ربط وتطبيق ومتابعة.'},
...
function printReport(){
 ...
 syncProfileInputs();
 const fileName=reportFileName();
 const logoUrl=new URL('assets/images/emtidad-logo.png?v=0.5.5',document.baseURI).href;
 const naifUrl=new URL('assets/images/naif-logo.png',document.baseURI).href;
 const regularUrl=new URL('assets/fonts/TheYearofHandicrafts-Regular.woff2',document.baseURI).href;
 const semiUrl=new URL('assets/fonts/TheYearofHandicrafts-SemiBold.woff2',document.baseURI).href;
 const blackUrl=new URL('assets/fonts/TheYearofHandicrafts-Black.woff2',document.baseURI).href;
...
async function buildResultImageCanvas(){
 ...
 const [logo,naif]=await Promise.all([
  loadCanvasImage(new URL('assets/images/emtidad-logo.png?v=0.5.5',document.baseURI).href),
...
function load(){try{const x=JSON.parse(localStorage.getItem(STORAGE_KEY)||'null');if(x){Object.assign(state,x);state.profile=state.profile||{};Object.keys(state.profile).forEach(key=>state.profile[key]=latinDigits(state.profile[key]));state.answers=state.answers||{}}}catch(e){}}
...
function renderKnowledge(){const q=$('knowledgeSearch').value.trim().toLowerCase(),type=$('knowledgeType').value,region=$('knowledgeRegion').value,level=$('knowledgeLevel').value;const rows=knowledgeData.filter(x=>(!q||[x.title,x.topic,x.source,x.summary,...x.highlights].join(' ').toLowerCase().includes(q))&&(!type||x.type===type)&&(!region||x.region===region)&&(!level||x.level===level));$('knowledgeGrid').innerHTML=rows.length?rows.map(x=>`<article class="knowledge-card ${x.featured?'featured':''}"><div class="meta"><span class="chip ${x.type.includes('نظام')?'official':''}">${x.type}</span><span class="chip">${x.region}</span><span class="chip">${x.level}</span></div><h3>${x.title}</h3><p>${x.summary}</p><ul class="highlights">${x.highlights.map(h=>`<li>${h}</li>`).join('')}</ul><footer><span>${x.source} · ${x.year} · مراجعة ${x.reviewed}</span>${x.url?`<a href="${x.url}" target="_blank" rel="noopener">المصدر ${icon('external')}</a>`:''}</footer></article>`).join(''):'<div class="empty-state">ستظهر هنا النتائج المطابقة لبحثك.</div>'}
...
function templateDocumentHtml(i,mode='print',embeddedImages={}){
 ...
 const logoUrl=embeddedImages.logo||new URL('assets/images/emtidad-logo.png?v=0.5.5',document.baseURI).href;
...
async function downloadTemplateWord(i){
 ...
 try{
  const [D,logo,naif]=await Promise.all([
   ensureDocxLibrary(),
   imageUrlAsBytes(new URL('assets/images/emtidad-logo.png?v=0.5.5',document.baseURI).href),
