...
const statsData=[
 {value:'83%',title:'رؤساء مجالس الإدارة من العائلة',meaning:'تمثيل العائلة يتركز في مستوى القرار الاستراتيجي لدى الشركات العائلية المدرجة المشمولة بالتقرير',region:'السعودية',sample:'الشركات العائلية المدرجة',year:'نوفمبر 2021',source:'هيئة السوق المالية',kind:'دراسة بحثية لهيئة السوق المالية',url:'https://cma.gov.sa/AboutCMA/ResearchAndReports/Documents/Development%20of%20family%20businesses.pdf'},
 {value:'32%',title:'مديرون تنفيذيون من العائلة',meaning:'تشير البيانات إلى زيادة الاستعانة بكفاءات من داخل العائلة وخارجها في الإدارة التنفيذية',region:'السعودية',sample:'الشركات العائلية المدرجة',year:'نوفمبر 2021',source:'هيئة السوق المالية',kind:'مؤشر مستخلص من دراسة هيئة السوق المالية',url:'https://cma.gov.sa/AboutCMA/ResearchAndReports/Documents/Development%20of%20family%20businesses.pdf'},
 {value:'25%',title:'من الشركات حققت نمو المبيعات بنسبة 10% فأكثر',meaning:'يربط التقرير النمو بالغرض المشترك والمرونة والاستثمار المدروس',region:'62 دولة ومنطقة',sample:'1,325 شركة عائلية',year:'2025',source:'PwC',kind:'مسح مهني',url:'https://www.pwc.com/gx/en/issues/business-model-reinvention/family-business-survey.html'},
...
const knowledgeData=[
 ...
 {id:'pwc2025',title:'الاستطلاع العالمي الثاني عشر للشركات العائلية',type:'تقرير مهني',topic:'النمو والتجديد',region:'عالمي',year:'2025',level:'متوسط',source:'PwC',summary:'مسح عالمي عن النمو والغرض والمرونة والاستثمار والتقنية بين الشركات العائلية',highlights:['1,325 شركة في 62 دولة ومنطقة','25% من الشركات حققت نمو المبيعات بنسبة 10% فأكثر','الغرض والمرونة يرتبطان بالأداء'],url:'https://www.pwc.com/gx/en/issues/business-model-reinvention/family-business-survey.html',reviewed:'يوليو 2026'},
...
const questions={
 ...
 company:[
  ...
  {id:'governance_without_board',q:'هل توجد آلية معتمدة للقرار والرقابة عند عدم وجود مجلس إدارة؟',d:'تحدد جهة القرار والصلاحيات والمتابعة بما يناسب شكل المنشأة وحجمها',w:3,doc:'إطار القرار والرقابة',action:'تحديد جهة القرار والرقابة وصلاحياتها وآلية متابعة قراراتها',show:p=>p.board==='none'},
...
function printReport(){
 ...
 const logoUrl=new URL('assets/images/emtidad-logo.png?v=0.6.9',document.baseURI).href;
 const naifUrl=new URL('assets/images/naif-logo-gold.png?v=0.6.9',document.baseURI).href;
 ...
 *{box-sizing:border-box;font-synthesis:none}
 ...
 <section class="columns"><article class="card"><h3>${resultSectionTitle('risk',r.risks)}</h3><ol>${list(r.risks,'risk')}</ol></article><article class="card"><h3>${resultSectionTitle('doc',r.docs)}</h3><ol>${list(r.docs,'doc')}</ol></article><article class="card"><h3>أهم الأولويات</h3><ol>${list(r.priorities,'priority')}</ol></article><article class="card"><h3>أدوات ونماذج مرتبطة بالنتيجة</h3><ol>${reportTools}</ol></article></section>
...
async function buildResultImageCanvas(){
 ...
 const [logo,naif,...socialIcons]=await Promise.all([
  loadCanvasImage(new URL('assets/images/emtidad-logo.png?v=0.6.9',document.baseURI).href),
  loadCanvasImage(new URL('assets/images/naif-logo-gold.png?v=0.6.9',document.baseURI).href),
...
function templateDocumentHtml(i,mode='print',embeddedImages={}){
 ...
 const logoUrl=embeddedImages.logo||new URL('assets/images/emtidad-logo.png?v=0.6.9',document.baseURI).href;
 const naifUrl=embeddedImages.naif||new URL('assets/images/naif-logo-gold.png?v=0.6.9',document.baseURI).href;
...
async function downloadTemplateWord(i){
 ...
 try{
  const [D,logo,naif]=await Promise.all([
   ...
   imageUrlAsBytes(new URL('assets/images/naif-logo-gold.png?v=0.6.9',document.baseURI).href)
  ]);
...
function profilePanel(){return `<div class="panel-title"><div class="axis-icon">${icon('profile')}</div><div><h3>ملف العائلة والشركة</h3><p>تضيف البيانات خلفية عن الحالة إلى التقرير والمسودة، وتحدد بعض الأسئلة التي تناسب العائلة والشركة</p></div></div><div class="form-grid">
 ...
 </div>${state.profile.waqfInterest==='yes'?'<div class="profile-adaptive-note">أضيف محور الثروة والوقف إلى مسار التشخيص</div>':''}<div class="panel-actions panel-actions-end"><button class="btn primary" id="nextBtn">بدء المحاور</button></div>`}
...
function questionCard(axisId,q,i){
 const key=axisId+'_'+q.id,current=Object.prototype.hasOwnProperty.call(state.answers,key)?state.answers[key]:'unset';
 const button=a=>{const val=a.value===null?'na':String(a.value),selected=(current===null&&a.value===null)||String(current)===String(a.value);return `<button type="button" class="answer ${selected?'selected':''}" data-answer-key="${key}" data-answer="${val}">${a.label}</button>`};
 const scale=answerOptions.filter(a=>a.value!==null).map(button).join(''),exception=button(answerOptions.find(a=>a.value===null));
 return `<article class="question-card"><div class="question-head"><div class="q-number">${i+1}</div><div><h4>${q.q}</h4><p>${q.d}</p></div></div><div class="answers"><div class="answer-scale">${scale}</div><div class="answer-exception">${exception}</div></div></article>`;
}
...
function bindPanel(){
 document.querySelectorAll('#diagPanel input,#diagPanel select').forEach(el=>el.addEventListener('input',()=>{const value=latinDigits(el.value);if(el.value!==value)el.value=value;state.profile[el.id]=value;state.result=null;save();if(el.id==='waqfInterest'){const hasAnswers=Object.keys(state.answers).length>0;state.step=0;renderDiagnostic();toast(hasAnswers?'تم تحديث مسار التشخيص، إجاباتك السابقة محفوظة':value==='yes'?'أضيف محور الثروة والوقف إلى التشخيص':'تم تحديث مسار التشخيص')}}));
...
function resultEmptyText(type){return type==='risk'?'الإجابات الحالية تعكس مستوى متقدم من التطبيق والمتابعة في المحاور المحتسبة':'الوثائق الأساسية تظهر ضمن مستوى متقدم بحسب الإجابات'}
function resultSectionTitle(type,arr){return arr.length?(type==='risk'?'أهم المخاطر والفجوات':'وثائق مقترحة'):(type==='risk'?'قراءة المخاطر والفجوات':'حالة الوثائق')}
function resultItemsMarkup(arr,type){
 if(!arr.length)return `<div class="result-empty"><strong>${resultEmptyText(type)}</strong><span>استمر في المراجعة الدورية واختبار التطبيق</span></div>`;
 return arr.map((x,i)=>`<div class="result-item"><div class="result-index">${i+1}</div><div><strong>${type==='risk'?x.q:type==='doc'?x.doc:x.action}</strong><span>${type==='risk'?`${x.axis} · مستوى الإجابة ${x.value} من 4`:type==='doc'?'وثيقة مقترحة للإنشاء أو التحديث':'مرتبة بحسب الأثر والحاجة إلى المعالجة'}</span></div></div>`).join('');
}
function reportListItems(arr,type){
 if(!arr.length)return `<li class="empty-item"><div><strong>${escapeHtml(resultEmptyText(type))}</strong><span>استمر في المراجعة الدورية واختبار التطبيق</span></div></li>`;
 return arr.slice(0,type==='risk'?4:5).map((x,i)=>`<li><b>${i+1}</b><div><strong>${escapeHtml(type==='risk'?x.q:type==='doc'?x.doc:x.action)}</strong><span>${escapeHtml(type==='risk'?x.axis:type==='doc'?'وثيقة مقترحة للإنشاء أو التحديث':'أولوية مقترحة وفق الأثر والإلحاح')}</span></div></li>`).join('');
}
function buildResult(){
 const axisScores=applicableAxes().map(a=>({...a,...scoreAxis(a.id)})).filter(x=>x.score!==null);
 if(!axisScores.length){state.result=null;save();toast('تعذر حساب النتيجة لأن جميع الإجابات اختير لها «لا ينطبق»، اختر إجابة من سلم التقييم في سؤال واحد على الأقل');return false}
 const overall=Math.round(axisScores.reduce((s,x)=>s+x.score,0)/axisScores.length);
 const answered=axisScores.reduce((s,x)=>s+x.answered,0),total=axisScores.reduce((s,x)=>s+x.total,0),confidence=Math.round(answered/Math.max(1,total)*100);
 const [label,desc]=stage(overall),all=[];
 axisScores.forEach(a=>applicableQuestions(a.id).forEach(q=>{const k=a.id+'_'+q.id;if(!Object.prototype.hasOwnProperty.call(state.answers,k)||state.answers[k]===null)return;all.push({axis:a.title,axisId:a.id,q:q.q,value:state.answers[k],w:q.w,doc:q.doc,action:q.action,severity:(4-state.answers[k])*q.w})}));
 all.sort((a,b)=>b.severity-a.severity);
 const risks=all.filter(x=>x.severity>0).slice(0,4);
 const docs=[...new Map(all.filter(x=>x.value<=2).map(x=>[x.doc,x])).values()].slice(0,5);
 let priorities=[...new Map(all.filter(x=>x.value<=2).map(x=>[x.action,x])).values()].slice(0,5);
 if(!priorities.length)priorities=[{axis:'جميع المحاور',axisId:'maintenance',q:'المراجعة الدورية',value:4,w:1,doc:'سجل المراجعة الدورية',action:'المحافظة على التطبيق ومراجعة القواعد واختبارها دوريا',severity:0}];
 const strongest=[...axisScores].sort((a,b)=>b.score-a.score)[0],weakest=[...axisScores].sort((a,b)=>a.score-b.score)[0];
 state.result={schemaVersion:RESULT_SCHEMA_VERSION,overall,confidence,label,desc,axisScores,risks,docs,priorities,strongest,weakest,valuation:valuationReadiness()};save();return true;
}
function resultPanel(){const r=state.result;if(!r||r.schemaVersion!==RESULT_SCHEMA_VERSION){if(!buildResult())return `<div class="result-empty"><strong>تعذر حساب النتيجة</strong><span>اختر إجابة من سلم التقييم في سؤال واحد على الأقل</span></div>`;return resultPanel()}const familyName=String(state.profile.familyName||'').trim();const familyMarkup=familyName?`<div class="result-entity"><small>اسم العائلة أو الشركة</small><strong>${escapeHtml(familyName)}</strong></div>`:'';const axisHtml=r.axisScores.map(a=>`<article class="axis-result"><div class="axis-result-head"><h4>${a.title}</h4><strong>${a.score}%</strong></div><div class="bar"><span style="width:${a.score}%"></span></div></article>`).join('');const list=resultItemsMarkup;return `<div class="panel-title"><div class="axis-icon">${icon('report')}</div><div><h3>نتيجة جاهزية الاستدامة</h3><p>نتيجة استرشادية من 100 تساعد على ترتيب الحوار والوثائق والأولويات</p></div></div><div class="result-shell"><section class="result-summary"><div class="score-ring" style="--score:${r.overall}"><div><strong>${r.overall}%</strong><span>جاهزية الاستدامة</span></div></div><div>${familyMarkup}<h3>${r.label}</h3><p>${r.desc}</p><div class="result-kpis"><div class="result-kpi"><small>نسبة الأسئلة المحتسبة</small><strong>${r.confidence}%</strong></div><div class="result-kpi"><small>أقوى محور</small><strong>${r.strongest.title}</strong></div><div class="result-kpi"><small>الأولوية الآن</small><strong>${r.weakest.title}</strong></div></div></div></section>${valuationResultMarkup(r.valuation)}<section class="axis-results">${axisHtml}</section><div class="result-grid"><section class="result-card"><h3>${resultSectionTitle('risk',r.risks)}</h3><div class="risk-list">${list(r.risks,'risk')}</div></section><section class="result-card"><h3>${resultSectionTitle('doc',r.docs)}</h3><div class="doc-list">${list(r.docs,'doc')}</div></section><section class="result-card"><h3>أهم الأولويات</h3><div class="priority-list">${list(r.priorities,'priority')}</div></section><section class="result-card"><h3>مواد تناسب نتيجة التشخيص</h3><div class="priority-list">${knowledgeForAxis(r.weakest.id).map((x,i)=>`<div class="result-item"><div class="result-index">${i+1}</div><div><strong>${x.title}</strong><span>${x.source} · ${x.year}</span></div></div>`).join('')}</div></section></div><div class="method-card"><strong>طريقة حساب النتيجة:</strong> يحسب كل محور من الأسئلة المناسبة لحالة العائلة بحسب أهمية السؤال، ثم يحسب المتوسط بين المحاور، يستبعد الحساب الأسئلة التي اختير لها «لا ينطبق»، ويضاف محور الوقف عند وجود وقف أو رغبة في دراسته، النتيجة استرشادية وتستخدم لترتيب الحوار والأولويات</div><div class="result-footer"><div><h3>حفظ النتيجة</h3><p>احفظ النتيجة كصورة، أو حمل التقرير بصيغة PDF.</p><div class="print-filename">اسم الملف المقترح: <strong>${escapeHtml(reportFileName())}</strong></div></div><div class="result-actions"><button class="btn ghost" id="restartBtn">إعادة التشخيص</button><button class="btn ghost" id="imageBtn">${icon('report')} حفظ النتيجة كصورة</button><button class="btn secondary" id="printBtn">${icon('print')} تحميل التقرير بصيغة PDF</button></div></div></div>`}
...
if($('modalClose')&&$('modal')){
 ...
 document.addEventListener('keydown',e=>{
  ...
  if(e.key==='Escape'){e.preventDefault();closeToolModal();return}
  if(e.key!=='Tab')return;
