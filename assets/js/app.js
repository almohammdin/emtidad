...
const questions={
 ...
 ownership:[
  ...
  {id:'exit_policy',q:'هل آلية التخارج وبيع الحصص أو الأسهم والتقييم واضحة؟',d:'حق الأولوية وطريقة التقييم وآلية الدفع.',w:3,doc:'سياسة التخارج والتقييم',action:'تصميم سياسة تخارج وتقييم تقلل المفاجآت والنزاع.'},
 ...
 company:[
  ...
  {id:'conflict',q:'هل سياسة تعارض المصالح والإفصاح مطبقة؟',d:'تشمل الأطراف ذات العلاقة والمصالح العائلية.',w:3,doc:'سياسة تعارض المصالح',action:'اعتماد سجل وإجراءات واضحة لتعارض المصالح.'}
 ],
 succession:[
...
function enhanceResultActions(){
 ...
 if(!$('resultServices')){
  const tools=['فحص صحة الحوكمة','منظومة أعمال المجالس','بوصلة النمو','سجل المخاطر'].map(title=>naifToolsData.find(tool=>tool.title===title)).filter(Boolean);
  const services=document.createElement('section');services.className='result-services';services.id='resultServices';
  services.innerHTML=`<div class="result-services-head"><div><span>أدوات تكمل النتيجة</span><h3>خطوات عملية مرتبطة بالتشخيص</h3></div></div><div class="result-services-grid">${tools.map(tool=>`<a href="${tool.url}" target="_blank" rel="noopener"><span class="service-icon">${icon(tool.icon)}</span><span><strong>${tool.title}</strong><small>${tool.desc}</small></span>${icon('external')}</a>`).join('')}</div>`;
...
function renderStats(){$('statsGrid').innerHTML=statsData.map(s=>`<article class="stat-card"><div class="stat-value">${s.value}</div><h3>${s.title}</h3><p>${s.meaning}</p><div class="source-line"><span>${s.region}</span><span>${s.sample}</span><span>${s.year}</span><a href="${s.url}" target="_blank" rel="noopener">${s.source}</a></div></article>`).join('')}
...
load();renderStats();renderAxesPreview();renderDiagnostic();initDeferredContent();
