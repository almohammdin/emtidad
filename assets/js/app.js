...
const knowledgeData=[
 ...
 {id:'family-business-waqf-khalid-alrajhi',featured:true,title:'الشركات العائلية والوقف',type:'لقاء مرئي',topic:'الشركات العائلية والوقف والوصايا',region:'سعودي',year:'2022',published:'7 أغسطس 2022',level:'متوسط',source:'مركز واقف [خبراء الوصايا والأوقاف] · د خالد بن عبد الرحمن الراجحي',summary:'لقاء يتناول علاقة الوقف بالشركات العائلية والاعتبارات التي تسبق وقف الأصول أو الحصص، ودور الترتيب الشرعي والنظامي في حماية الغرض واستمرار الأعمال',highlights:['العلاقة بين الشركة العائلية والوقف','وقف الحصص والأصول العائلية','أثر الترتيب المبكر على الاستمرار'],url:'https://www.youtube.com/watch?v=puZIjyBuKDo',videoId:'puZIjyBuKDo',actionLabel:'فتح اللقاء في يوتيوب'},
...
function reportDate(){
 const now=new Date();
 const dayName=new Intl.DateTimeFormat('ar-SA-u-ca-gregory-nu-latn',{weekday:'long'}).format(now);
 const hijriDate=new Intl.DateTimeFormat('ar-SA-u-ca-islamic-umalqura-nu-latn',{day:'numeric',month:'long',year:'numeric',era:'short'}).format(now);
 const gregorianDate=new Intl.DateTimeFormat('ar-SA-u-ca-gregory-nu-latn',{day:'numeric',month:'long',year:'numeric',era:'short'}).format(now);
 return `${dayName} | ${hijriDate} | ${gregorianDate}`;
}
...
function printReportLegacy(){
 ...
 const naifUrl=new URL('assets/images/naif-logo-gold.png?v=0.6.7',document.baseURI).href;
...
function printReport(){
 ...
 syncProfileInputs();
 const fileName=reportFileName();
 const logoUrl=new URL('assets/images/emtidad-logo.png?v=0.6.7',document.baseURI).href;
 const naifUrl=new URL('assets/images/naif-logo-gold.png?v=0.6.7',document.baseURI).href;
 const regularUrl=new URL('assets/fonts/TheYearofHandicrafts-Regular.woff2',document.baseURI).href;
 const semiUrl=new URL('assets/fonts/TheYearofHandicrafts-SemiBold.woff2',document.baseURI).href;
 const blackUrl=new URL('assets/fonts/TheYearofHandicrafts-Black.woff2',document.baseURI).href;
 ...
 *{box-sizing:border-box;font-synthesis:none}
 ...
 .meta{min-width:225px;text-align:left;font-size:8px;color:#6B7780;display:grid;gap:2px;justify-items:end}.meta strong{display:block;color:#0D3656;font-size:10.5px}.meta span{direction:rtl;white-space:nowrap}
...
function canvasSvgIcon(path,color='#C9853C'){
...
async function buildResultImageCanvas(){
 ...
 const [logo,naif,...socialIcons]=await Promise.all([
  loadCanvasImage(new URL('assets/images/emtidad-logo.png?v=0.6.7',document.baseURI).href),
  loadCanvasImage(new URL('assets/images/naif-logo-gold.png?v=0.6.7',document.baseURI).href),
  ...socialPaths.map(path=>canvasSvgIcon(path))
 ]);
 const canvas=document.createElement('canvas');
 canvas.width=2160;canvas.height=2700;
 const ctx=canvas.getContext('2d',{alpha:false});
 ctx.scale(1.35,1.35);ctx.imageSmoothingEnabled=true;ctx.imageSmoothingQuality='high';
 const navy='#0D3656',gold='#C9853C',ink='#263843',muted='#68747D',paper='#F7F5F1';
 ...
 ctx.fillStyle=gold;ctx.font=`700 22px ${font}`;
 ctx.fillText(family,800,440);
 ctx.fillStyle=muted;ctx.font=`600 18px ${font}`;
 ctx.fillText(reportDate(),800,475);
 ...
 canvasImageContain(ctx,naif,1260,1890,240,72);
...
function renderKnowledge(){
 const q=$('knowledgeSearch').value.trim().toLowerCase(),type=$('knowledgeType').value,region=$('knowledgeRegion').value,level=$('knowledgeLevel').value;
 const rows=knowledgeData.filter(x=>(!q||[x.title,x.topic,x.source,x.summary,...x.highlights].join(' ').toLowerCase().includes(q))&&(!type||x.type===type)&&(!region||x.region===region)&&(!level||x.level===level));
 $('knowledgeGrid').innerHTML=rows.length?rows.map(x=>{
  const publication=x.published?`تاريخ النشر ${x.published}`:[x.year,x.reviewed?`مراجعة ${x.reviewed}`:''].filter(Boolean).join(' · ');
  const meta=`<div class="meta"><span class="chip ${x.type.includes('نظام')?'official':''}">${x.type}</span><span class="chip">${x.region}</span><span class="chip">${x.level}</span></div>`;
  const highlights=`<ul class="highlights">${x.highlights.map(h=>`<li>${h}</li>`).join('')}</ul>`;
  const footer=`<footer><span>${x.source} · ${publication}</span>${x.url?`<a href="${x.url}" target="_blank" rel="noopener">${x.actionLabel||'فتح المرجع'} ${icon('external')}</a>`:''}</footer>`;
  if(x.videoId)return `<article class="knowledge-card has-video"><div class="knowledge-video-copy">${meta}<h3>${x.title}</h3><p>${x.summary}</p>${highlights}${footer}</div><div class="knowledge-video"><iframe src="https://www.youtube-nocookie.com/embed/${x.videoId}" title="${x.title}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div></article>`;
  return `<article class="knowledge-card ${x.featured?'featured':''}">${meta}<h3>${x.title}</h3><p>${x.summary}</p>${highlights}${footer}</article>`;
 }).join(''):'<div class="empty-state">لا توجد نتائج مطابقة لبحثك</div>'
}
function renderCases(){
 $('caseGrid').innerHTML=caseStudiesData.map(c=>{
  const resource=c.resource?`<aside class="case-resource"><span>مادة تستحق القراءة</span><strong>${c.resource.title}</strong><p>${c.resource.summary}</p><a href="${c.resource.url}" target="_blank" rel="noopener">فتح الكتاب ${icon('external')}</a></aside>`:'';
  const companion=c.media?.companion?`<aside class="knowledge-companion case-companion"><span>${c.media.companion.eyebrow}</span><strong>${c.media.companion.title}</strong><p>${c.media.companion.summary}</p><small>${c.media.companion.credit}</small><a href="${c.media.companion.url}" target="_blank" rel="noopener">${c.media.companion.actionLabel} ${icon('external')}</a></aside>`:'';
  const caseTop=`<div class="case-top"><span>${c.focus}</span><small>${c.reviewed}</small></div><h3>${c.name}</h3><p>${c.summary}</p><ul class="case-lessons">${c.lessons.map(x=>`<li>${x}</li>`).join('')}</ul>`;
  const caseSource=`<a class="case-source-link" href="${c.url}" target="_blank" rel="noopener">${c.source} ${icon('external')}</a>`;
  if(c.media){
   const media=`<section class="case-media"><div class="case-media-head"><span>${c.media.eyebrow}</span><h4>${c.media.title}</h4><p>${c.media.summary}</p><div class="case-media-meta"><span>${c.media.source} · تاريخ النشر ${c.media.published}</span><a href="${c.media.url}" target="_blank" rel="noopener">فتح الحلقة في يوتيوب ${icon('external')}</a></div></div><div class="case-media-body"><div class="knowledge-video"><iframe src="https://www.youtube-nocookie.com/embed/${c.media.videoId}" title="${c.media.title}" loading="lazy" referrerpolicy="strict-origin-when-cross-origin" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe></div>${companion}</div></section>`;
   return `<article class="case-card featured has-media"><div class="case-feature-layout"><section class="case-overview">${caseTop}${caseSource}</section>${media}</div></article>`
  }
  return `<article class="case-card ${c.featured?'featured':''}">${caseTop}${resource}${caseSource}</article>`
 }).join('')
}
...
function templateDocumentHtml(i,mode='print',embeddedImages={}){
 ...
 const logoUrl=embeddedImages.logo||new URL('assets/images/emtidad-logo.png?v=0.6.7',document.baseURI).href;
 const naifUrl=embeddedImages.naif||new URL('assets/images/naif-logo-gold.png?v=0.6.7',document.baseURI).href;
...
async function downloadTemplateWord(i){
 ...
 try{
  const [D,logo,naif]=await Promise.all([
   ...
   imageUrlAsBytes(new URL('assets/images/naif-logo-gold.png?v=0.6.7',document.baseURI).href)
  ]);
...
function knowledgeForAxis(axisId){const map={purpose:['pwc2025','imd'],charter:['law11','qadha-charter-2026','fbcg-code'],ownership:['ncfb-valuation-2026','law11'],company:['ncfb-valuation-2026','ifc'],succession:['ifc','insead'],wealth:['rajhiwaqf','family-business-waqf-khalid-alrajhi'],resilience:['ncfb-valuation-2026','kpmg2025']};return (map[axisId]||['ifc','pwc2025']).map(id=>knowledgeData.find(x=>x.id===id)).filter(Boolean)}
