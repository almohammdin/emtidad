...
try{
  ...
  if(Array.isArray(raw)){
    people=raw.filter(p=>p&&p.id&&p.name).map(p=>({
    ...
    if(!localStorage.getItem(K)&&people.length){
      localStorage.setItem(K,JSON.stringify(people));
      localStorage.setItem(K+'_updated',new Date().toISOString());
    }
  }
}catch{}
...
function cardHtml(p){
  ...
  const years=p.birthYear&&p.deathYear?`${p.birthYear} – ${p.deathYear}`:p.birthYear?`مواليد ${p.birthYear}`:p.deathYear?`توفي ${p.deathYear}`:'';
  return `<div class="ft-head"><span class="ft-avatar ${p.gender==='female'?'f':''}">${esc((p.name||'?').charAt(0))}</span><div class="ft-name"><b>${esc(p.name)}</b><span>${esc(years)}</span></div></div><div class="ft-tags">${spouses.length?`<span>زوج: ${esc(spouses.join('، '))}</span>`:''}${childCount?`<span>${childCount} من الأبناء</span>`:''}${p.branch?`<span>${esc(p.branch)}</span>`:''}</div>`;
}
function render(){
  ...
  empty.hidden=true;const values=people.map(generation),max=Math.max(...values),map=new Map();
  ...
  for(let g=0;g<=max;g++){
    ...
    (map.get(g)||[]).sort((a,b)=>a.name.localeCompare(b.name,'ar')).forEach(p=>{
...
function center(el){const r=el.getBoundingClientRect(),c=$('canvas').getBoundingClientRect();return {x:r.left-c.left+r.width/2,y:r.top-c.top+r.height/2,top:r.top-c.top,bottom:r.bottom-c.top};}
function addPath(a,b,cls=''){const p=document.createElementNS('http://www.w3.org/2000/svg','path'),mid=(a.y+b.y)/2;p.setAttribute('d',`M ${a.x} ${a.y} C ${a.x} ${mid}, ${b.x} ${mid}, ${b.x} ${b.y}`);if(cls)p.setAttribute('class',cls);$('lines').appendChild(p);}
function drawLines(){
  const svg=$('lines'),canvas=$('canvas');svg.innerHTML='';svg.setAttribute('viewBox',`0 0 ${canvas.scrollWidth} ${canvas.scrollHeight}`);const seen=new Set();
  people.forEach(p=>{
    const child=document.querySelector(`.ft-person[data-id="${CSS.escape(p.id)}"]`);if(!child)return;
    [p.fatherId,p.motherId].filter(Boolean).forEach(pid=>{const par=document.querySelector(`.ft-person[data-id="${CSS.escape(pid)}"]`);if(par){const a=center(par),b=center(child);addPath({x:a.x,y:a.bottom},{x:b.x,y:b.top});}});
    (p.spouseIds||[]).forEach(sid=>{const key=[p.id,sid].sort().join('|');if(seen.has(key))return;seen.add(key);const spouse=document.querySelector(`.ft-person[data-id="${CSS.escape(sid)}"]`);if(spouse){const a=center(child),b=center(spouse),line=document.createElementNS('http://www.w3.org/2000/svg','path');line.setAttribute('d',`M ${a.x} ${(a.y+b.y)/2} L ${b.x} ${(a.y+b.y)/2}`);line.setAttribute('class','spouse');svg.appendChild(line);}});
  });
}
...
function fillRelations(currentId=''){
  ...
  $('father').innerHTML='<option value="">—</option>'+optionList('male',currentId);
  $('mother').innerHTML='<option value="">—</option>'+optionList('female',currentId);
  $('spouses').innerHTML=people.filter(p=>p.id!==currentId).sort((a,b)=>a.name.localeCompare(b.name,'ar')).map(p=>`<option value="${p.id}">${esc(p.name)}</option>`).join('');
...
function openModal(id=''){
  const p=get(id);$('modalTitle').textContent=p?'تعديل فرد':'إضافة فرد';$('id').value=p?.id||'';$('name').value=p?.name||'';$('gender').value=p?.gender||'male';$('birth').value=p?.birthYear||'';$('death').value=p?.deathYear||'';$('branch').value=p?.branch||'';$('notes').value=p?.notes||'';fillRelations(id);$('deleteBtn').hidden=!p;$('quickChildBtn').hidden=!p;$('modal').hidden=false;setTimeout(()=>$('name').focus(),30);
}
...
function submit(e){
  e.preventDefault();const id=$('id').value||uid(),fatherId=$('father').value,motherId=$('mother').value;
  if(fatherId&&motherId&&fatherId===motherId){alert('الأب والأم يجب أن يكونا شخصين مختلفين');return;}
  if(wouldCycle(id,fatherId)||wouldCycle(id,motherId)){alert('هذه العلاقة ستنشئ دورة داخل الشجرة. راجع اختيار الأب أو الأم.');return;}
  let p=get(id);if(!p){p={id,spouseIds:[]};people.push(p);}
  p.name=$('name').value.trim();p.gender=$('gender').value;p.birthYear=$('birth').value.trim();p.deathYear=$('death').value.trim();p.fatherId=fatherId;p.motherId=motherId;p.branch=$('branch').value.trim();p.notes=$('notes').value.trim();syncSpouses(id,selectedValues($('spouses')));save();closeModal();
}
...
function sample(){
  ...
  const f={id:uid(),name:'فيصل',gender:'male',birthYear:'2002',deathYear:'',fatherId:c.id,motherId:'',branch:'فرع سعد',notes:'',spouseIds:[]};
  const g={id:uid(),name:'سارة',gender:'female',birthYear:'2005',deathYear:'',fatherId:c.id,motherId:'',branch:'فرع سعد',notes:'',spouseIds:[]};
  people=[a,b,c,d,e,f,g];save();
}
