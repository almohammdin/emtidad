...
const CHARTER_OPTION_LABELS={
 ...
 owners:{'1-3':'1 إلى 3 ملاك','4-10':'4 إلى 10 ملاك','11-30':'11 إلى 30 مالكاً','31+':'أكثر من 30 مالكاً'},
 ...
 decisionModel:{majority:'أغلبية الحاضرين',enhanced:'أغلبية عادية و75% للقرارات الحساسة',consensus:'التوافق أولاً ثم التصويت عند تعذره'},
 ...
 valuationPolicy:{one:'مقيم معتمد مستقل',twoAverage:'متوسط تقييم مقيمين معتمدين',agreed:'منهج تقييم معتمد مسبقاً بحسب سبب التقييم'},
...
function charterIdentityMarkup(){
 const diagnosticStatus=charterHasDiagnostic()
  ?'<div class="charter-diagnostic-status linked"><strong>نتيجة التشخيص متاحة</strong><span>يمكن استدعاء بيانات الملف وتخصيص تنبيهات المسودة.</span><button type="button" data-charter-action="refresh-diagnostic">تحديث من التشخيص</button></div>'
  :'<div class="charter-diagnostic-status"><strong>المسودة المباشرة</strong><span>أكمل البيانات التالية، أو نفذ التشخيص أولاً للحصول على تخصيص أعمق.</span><a href="#diagnostic">فتح التشخيص</a></div>';
...
function charterFamilyMarkup(){
 ...
 return `${charterStepIntro('المرحلة 2','هوية العائلة وحوكمتها','ثبّت الغرض والقيم وهيئة الحوار واتخاذ القرار قبل تحويلها إلى مواد.')}
 <div class="charter-form-grid">
  ...
  ${charterSelect('decisionModel','طريقة اتخاذ القرار',[
   ['majority','أغلبية الحاضرين'],['enhanced','أغلبية عادية و75% للقرارات الحساسة'],['consensus','التوافق أولاً ثم التصويت عند تعذره']
  ],true)}
 ...
 <div class="charter-decision-note"><strong>القاعدة المستخدمة في الصياغة</strong><span>تظل أجهزة الشركة النظامية واختصاصاتها مستقلة، ويعمل مجلس العائلة كهيئة حوكمة عائلية وفق النطاق المعتمد له.</span></div>`;
}
function charterOwnershipMarkup(){
 ...
 <div class="charter-form-grid">
  ...
  ${charterSelect('valuationPolicy','طريقة التقييم عند التخارج',[
   ['one','مقيم معتمد مستقل'],['twoAverage','متوسط تقييم مقيمين معتمدين'],['agreed','منهج تقييم معتمد مسبقاً بحسب سبب التقييم']
  ],true)}
...
function charterWarnings(data,profile){
 ...
 if(!charterState.linkedDiagnostic)push('التخصيص التشخيصي','أُعدت المسودة من اختيارات هذا المسار. تنفيذ التشخيص يضيف قراءة للفجوات والأولويات.','info');
 ...
 if(profile.board==='none'||profile.board==='informal')push('مجلس الإدارة','مواد المجلس في هذه المسودة تمثل وضعاً مستهدفاً، ويجب مواءمتها مع الهيكل النظامي القائم.','attention');
 ...
 if(data.legalForm==='other')push('الشكل النظامي','تحتاج المصطلحات والاختصاصات إلى تخصيص قانوني قبل الاعتماد.','attention');
...
function charterBuildDraft(){
 ...
 chapters.push(charterChapter('الباب الأول: الأحكام العامة ونطاق الميثاق',[
  charterArticle('طبيعة الوثيقة',`هذه الوثيقة مسودة استرشادية لميثاق عائلة ${family} المالكة أو المرتبطة بشركة ${company}. تصبح صيغتها النهائية قابلة للاعتماد بعد مناقشتها ومواءمتها مع الأنظمة و${t.doc} وصدورها من الجهة المختصة.`,['law11','qadha'],'يحتاج مراجعة نظامية'),
  charterArticle('التعريفات',[
   ...
   `عضو العائلة: كل شخص تتحقق فيه صفة الانتماء العائلي وفق التعريف الذي تعتمده العائلة، سواء كان مالكاً أو عاملاً أو مستفيداً من برامج العائلة.`,
 ...
 chapters.push(charterChapter('الباب الثاني: هوية العائلة والغرض المشترك',[
  ...
  charterArticle('حقوق أعضاء العائلة ومسؤولياتهم',[
   'يحصل أعضاء العائلة على المعلومات والتثقيف اللازمين لممارسة أدوارهم بصفتهم ملاكاً أو أعضاء في هيئات العائلة وفق الصلاحيات المعتمدة.',
   'يلتزم عضو العائلة بحماية سمعة العائلة والشركة، واحترام القرارات المؤسسية، وفصل مصلحته الشخصية عن مصلحة الشركة.',
   'تراعى العدالة بين الفروع والأجيال مع بقاء الكفاءة والاختصاص أساساً للأدوار المهنية والتنفيذية.'
  ],['gcc','ifc'],'أساسي'),
 ...
 const familyGovernanceArticles=[
  ...
  charterArticle('الجمعية العائلية',`تضم الجمعية العائلية أفراد العائلة المشمولين وفق ضوابط العضوية، وتعقد اجتماعاً دورياً للتواصل والتثقيف ومناقشة القيم والملكية والتعاقب وانتخاب ممثلي العائلة متى كان ذلك داخلاً في اختصاصها.`,['gcc','ifc'],'أساسي'),
 ...
 const transferText={
  familyFirst:`يعرض المالك الراغب في التصرف في ${t.unit} رغبته أولاً على بقية ${t.owners} من العائلة وفق آلية إخطار ومدد وسعر تحدد في سياسة مستقلة ومتسقة مع ${t.doc}.`,
 ...
 const valuationText={
  ...
  twoAverage:'يستند السعر إلى متوسط تقييم مقيمين معتمدين مستقلين، مع آلية لحسم الفروق الجوهرية وتحديد من يتحمل التكلفة.',
  agreed:'تقر العائلة منهج تقييم مسبقاً لكل حالة جوهرية، ويطبقه مقيم معتمد مستقل في تاريخ التقييم المحدد.'
 }[d.valuationPolicy];
 const dividendText={
  ...
  annual:'يعتمد قرار التوزيع سنوياً في ضوء النتائج والتدفقات والالتزامات وخطة الاستثمار، وفق اختصاص الجهة النظامية.'
 }[d.dividendPolicy];
 ...
 if(p.waqfInterest==='yes'){
  ownershipArticles.push(
   ...
   charterArticle('إدارة الثروة العائلية',`تفصل العائلة بين أصول الشركة وأصول الوقف والثروة الشخصية، وتحدد عند الحاجة نطاق مكتب العائلة وخدماته وحوكمته وتقاريره وسياسة الاستثمار وإدارة المخاطر.`,['gcc','ifc','diagnostic'],'مخصص من التشخيص')
  );
 }
 ...
 const boardText={
  ...
  mixed:'يستهدف تشكيل المجلس مزيجاً متوازناً من أفراد العائلة المؤهلين والأعضاء المستقلين وذوي الخبرة الملائمة لأعمال الشركة.',
 ...
 const companyArticles=[
  ...
  (d.legalForm==='holding'||p.holding==='yes')?charterArticle('حوكمة الشركات التابعة',`تعتمد المجموعة إطاراً يوضح صلاحيات الشركة القابضة ومجالس الشركات التابعة وإداراتها، وسياسة التمثيل والتقارير والمعاملات البينية وتعارض المصالح، مع احترام الشخصية النظامية لكل شركة.`,['gcc','diagnostic'],'مخصص من التشخيص'):null
 ];
 chapters.push(charterChapter('الباب الخامس: حوكمة الشركة والإدارة',companyArticles));
 const employmentText={
  merit:'يكون التوظيف قائماً على حاجة وظيفية معتمدة والجدارة والمنافسة العادلة، ولا تنشأ وظيفة لغرض استيعاب فرد من العائلة.',
  limited:'تحصر الوظائف المتاحة لأفراد العائلة ضمن فئات ومسارات تعتمد مسبقاً، ويظل شغلها مرتبطاً بالجدارة واحتياج العمل.',
 ...
 const experienceText=d.externalExperience==='0'
  ...
  :`يشترط للتوظيف الدائم خبرة عملية لا تقل عن ${charterLabel('externalExperience',d.externalExperience)}، في مجال ذي صلة، مع إثبات أداء وتطور مهني مناسبين.`;
 ...
 const successionText={
  ...
  readiness:'تعتمد معايير جاهزية لكل دور قيادي وقائمة مرشحين داخلية وخارجية، وتراجع الجاهزية دورياً بواسطة المجلس.',
  emergency:'تعتمد خطة انتقال طارئ للصلاحيات واستمرار الأعمال، إلى جانب خطة طويلة الأجل لاختيار الخلف وتأهيله وتسليم الدور.'
 }[d.successionPolicy];
 const nextGenText={
  program:'تنفذ العائلة برنامجاً سنوياً للتثقيف بالملكية والحوكمة والقراءة المالية والقيم وزيارات الأعمال.',
  mentoring:'يرتبط أفراد الجيل القادم بمرشدين وتتاح لهم معايشة منظمة للقيادات والمجالس دون منح صلاحيات قبل استيفاء المعايير.',
  ownership:'يستكمل المالك الجديد برنامجاً تعريفياً بحقوقه وواجباته والميثاق ووثائق الشركة قبل المشاركة في هيئات العائلة.'
 }[d.nextGen];
 chapters.push(charterChapter('الباب السادس: العمل العائلي والتعاقب بين الأجيال',[
  ...
  charterArticle('المساءلة وانتهاء الخدمة',`يخضع فرد العائلة للسياسات المهنية نفسها المطبقة على شاغلي الوظائف المماثلة، وتفصل معالجة الأداء أو إنهاء الخدمة عن حقوقه بصفته مالكاً أو عضواً في العائلة.`,['gcc','ifc'],'أساسي'),
  charterArticle('التعاقب القيادي',successionText,['gcc','ifc',...(charterState.linkedDiagnostic?['diagnostic']:[])],p.leadershipTransition==='near'?'أولوية حرجة':'اختيار العائلة'),
 ...
 const disputeText={
  ...
  committee:'تعرض المسألة أولاً على لجنة توفيق محايدة لا تضم طرفاً في الخلاف، ثم تنتقل إلى وسيط مستقل، مع بقاء الحقوق النظامية للأطراف.',
 ...
 chapters.push(charterChapter('الباب السابع: معالجة الخلافات والسرية',[
  ...
  charterArticle('الحياد وتعارض المصالح',`يمتنع عضو مجلس العائلة أو لجنة التوفيق عن المشاركة إذا كان طرفاً في الخلاف أو ذا مصلحة فيه، ويختار البديل وفق معايير الحياد والسرية والخبرة.`,['qadha','gcc'],'أساسي'),
 ...
 const notesText=d.notes.trim()?`وتسجل العائلة للمراجعة الموضوعات الآتية: ${d.notes.trim()}`:'وتحصر القرارات المؤجلة والموضوعات غير المحسومة في سجل مراجعة مستقل يبين المسؤول والموعد والمخرج المطلوب.';
 chapters.push(charterChapter('الباب الثامن: التنفيذ والمراجعة والأحكام الختامية',[
  charterArticle('خطة التنفيذ',`بعد اعتماد الميثاق، تحول مواده إلى سياسات ولوائح ومسؤوليات ومؤشرات متابعة. ويعد مجلس العائلة أو الجهة المكلفة تقريراً دورياً عن المواد المطبقة والمتعثرة وفرص التطوير.`,['qadha','gcc'],'أساسي'),
...
function openCharterBuilder(source='service'){
 ...
 toast(charterState.linkedDiagnostic?'تم استدعاء بيانات التشخيص':'ابدأ بإكمال بيانات العائلة');
}
...
function charterInjectResultCta(){
 ...
 cta.innerHTML=`<div><span>الخطوة التالية</span><h3>حوّل نتيجة التشخيص إلى مسودة ميثاق عائلي</h3><p>ستنتقل بيانات الملف والفجوات ذات الصلة تلقائياً إلى منشئ المسودة.</p></div><button class="btn primary" type="button" data-open-charter="result">إنشاء مسودة الميثاق</button>`;
